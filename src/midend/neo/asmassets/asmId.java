...
public class asmId {
  ...
  public irId ir = null;
  public int size = 4;  // 在栈上的大小
  public int space = 4; // 在堆上或全局元素的大小
  ...
  public asmId(irId ir, RegName re) {
    assert (!idMap.containsKey(ir));
    idMap.put(ir, this);
    this.ir = ir;
    this.asmType = AsmType.Register;
    this.size = ir.valueType.sizeof();
    this.reg = re;
  }

  public asmId(irId ir, int of) {
    assert (!idMap.containsKey(ir));
    idMap.put(ir, this);
    this.ir = ir;
  ...
  public void setGlobalPart(asmId orig, boolean lo) {
